...
class VTFLib:
    ...
    ImageCreate.argtypes = [c_int32,c_int32,c_int32,c_int32,c_int32,VTFLibEnums.ImageFormat,c_bool,c_bool,c_bool]
    ...
    def convert_to_rgba8888(self):
        new_size = self.compute_image_size(self.width(),self.height(),self.depth(),self.mipmap_count(),VTFLibEnums.ImageFormat.ImageFormatRGBA8888)
        new_buffer = cast((c_byte*new_size)(),POINTER(c_byte))
        # print('Input format:',self.image_format())
        if not self.ImageConvertToRGBA8888(self.ImageGetData(0,0,0,0), new_buffer, self.width(), self.height(),self.image_format().value):
    ...
    def convert(self,format):
        ...
        new_buffer = cast((c_byte*new_size)(),POINTER(c_byte))
        if not self.ImageConvert(self.ImageGetData(0,0,0,0), new_buffer, self.width(), self.height(),self.image_format().value,format):
            return self.pointer_to_array(new_buffer,new_size)
        else:
            sys.stderr.write('CAN\'T CONVERT IMAGE\n')
            return 0
...
if __name__ == '__main__':
    a = VTFLib()
    a.image_load(r"G:\SteamLibrary\SteamApps\common\SourceFilmmaker\game\usermod\materials\models\skuddbutt\mavis\body_clothed.vtf",False)
